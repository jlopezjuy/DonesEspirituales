...
import ar.com.dones.app.service.ResultadoDonQueryService;
...
public class ResultadoDonResource {
    ...
    private final ResultadoDonQueryService resultadoDonQueryService;

    public ResultadoDonResource(
        ResultadoDonService resultadoDonService,
        ResultadoDonRepository resultadoDonRepository,
        ResultadoDonQueryService resultadoDonQueryService
    ) {
        this.resultadoDonService = resultadoDonService;
        this.resultadoDonRepository = resultadoDonRepository;
        this.resultadoDonQueryService = resultadoDonQueryService;
    }
     ...
     */
    @PostMapping("")
    public ResponseEntity<ResultadoDonDTO> createResultadoDon(@Valid @RequestBody ResultadoDonDTO resultadoDonDTO)
     ...
     */
    @GetMapping("")
    public ResponseEntity<List<ResultadoDonDTO>> getAllResultadoDons(
        ResultadoDonCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get ResultadoDons by criteria: {}", criteria);

        Page<ResultadoDonDTO> page = resultadoDonQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /resultado-dons/count} : count all the resultadoDons.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countResultadoDons(ResultadoDonCriteria criteria) {
        LOG.debug("REST request to count ResultadoDons by criteria: {}", criteria);
        return ResponseEntity.ok().body(resultadoDonQueryService.countByCriteria(criteria));
    }
