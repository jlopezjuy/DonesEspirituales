...
type RestOf<T extends IUsuario | NewUsuario> = Omit<T, 'fechaNacimiento' | 'fechaRegistro' | 'ultimaActividad'> & {
...
export class UsuarioService {
  ...
  protected convertDateFromClient<T extends IUsuario | NewUsuario | PartialUpdateUsuario>(usuario: T): RestOf<T> {
    return {
      ...usuario,
      fechaNacimiento: usuario.fechaNacimiento?.format(DATE_FORMAT) ?? null,
      fechaRegistro: usuario.fechaRegistro?.toJSON() ?? null,
      ultimaActividad: usuario.ultimaActividad?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restUsuario: RestUsuario): IUsuario {
    return {
      ...restUsuario,
      fechaNacimiento: restUsuario.fechaNacimiento ? dayjs(restUsuario.fechaNacimiento) : undefined,
      fechaRegistro: restUsuario.fechaRegistro ? dayjs(restUsuario.fechaRegistro) : undefined,
      ultimaActividad: restUsuario.ultimaActividad ? dayjs(restUsuario.ultimaActividad) : undefined,
    };
  }
